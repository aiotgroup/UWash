...
if __name__ == "__main__":
    mat_file = scio.loadmat("/home/wuxilei/data/UTWash/brush_teeth/brush_teeth.mat")
    dataset = NormalDataset(mat_file['acc'], mat_file['gyr'], mat_file['label'])
    ...
    for batch_x, batch_y in tqdm(loader, desc='Evaluating'):
        with torch.no_grad():
            batch_x, batch_y = eval_preprocess(batch_x, batch_y, True)

            prediction = model(batch_x)

            prediction = eval_postprocess(prediction, True)
    ...
    showResult(result_gt, result_none, 10)
    print("vote".center(100, "="))
    showResult(result_gt, result_vote, 10)
    print("smooth".center(100, "="))
    showResult(result_gt, result_smooth, 10)
    print("both".center(100, "="))
    showResult(result_gt, result_both, 10)
