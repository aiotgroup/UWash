...
from UWasher.model.SPPUnet import UWasher, FocalLoss
...
def pickup_training(model, now_epoch, train_config, dataset_config, chose_device):
    ...
    max_eval_acc = 0
    print("开始训练")
    for epoch in range(now_epoch, train_config.num_epochs):
        # 首先训练
        model.train()
...
def train_eval_procedure(train_config, dataset_config, chose_device):
    os.environ['CUDA_VISIBLE_DEVICES'] = chose_device
...
def normal_eval_procedure(eval_config, dataset_config, chose_device):
    os.environ['CUDA_VISIBLE_DEVICES'] = chose_device
    ...
    def _get_mode(data):
        ret = []
        for ret_sample in data:
            mode = np.argmax(np.bincount(ret_sample), keepdims=True)[0]
            ret.append(mode)
        return np.array(ret)
    ...
    for i in range(len(indexes) - 1):
        ...
        total_count += n
        # correct_count += np.sum(_get_mode(temp_label) == _get_mode(temp_prediction))
    ...
    print("Coarse Accuracy: %.4f." % (correct_count / total_count))
