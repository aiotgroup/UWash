...
def train_preprocess(batch_x, batch_y, cuda):
    ...
    return torch.cat([batch_acc, batch_gyr], dim=1), torch.mode(batch_y, dim=1).values
...
def eval_preprocess(batch_x, batch_y, cuda):
    ...
    if cuda:
        ...
        batch_gyr = batch_gyr.type(torch.FloatTensor)
        batch_y = batch_y.type(torch.LongTensor)
    return torch.cat([batch_acc, batch_gyr], dim=1), batch_y


def eval_postprocess(prediction, cuda):
    prediction = prediction.max(dim=1)[1]
    prediction = prediction.unsqueeze(1).repeat(1, 64)
    return prediction
...
if __name__ == '__main__':
    ...
    if is_train:
        ...
        eval_config.index = 0
        eval_config.filename_model = "resnet1d_18-390-epochs-EvalAcc-88.96.pth"
