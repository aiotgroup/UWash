...
def train_preprocess(batch_x, batch_y, cuda):
    ...
    return (batch_acc.unsqueeze(-1), batch_gyr.unsqueeze(-1)), torch.mode(batch_y, dim=1).values
...
def eval_preprocess(batch_x, batch_y, cuda):
    ...
    if cuda:
        ...
        batch_gyr = batch_gyr.type(torch.FloatTensor)
        batch_y = batch_y.type(torch.LongTensor)
    return (batch_acc.unsqueeze(-1), batch_gyr.unsqueeze(-1)), batch_y
...
def train_eval_procedure(model_name, train_config, dataset_config, chose_device):
...
if __name__ == '__main__':
    ...
    if is_train:
        ...
        eval_config.index = 0
        eval_config.filename_model = "mobilenetv3_small-370-epochs-EvalAcc-87.19.pth"
